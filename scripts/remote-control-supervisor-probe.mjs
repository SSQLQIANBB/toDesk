...
if (!python || !process.argv.includes('--source=screen')) throw new Error('Requires REMOTE_CONTROL_PROBE_PYTHON and explicit --source=screen');
const mode = process.argv.find(value => value.startsWith('--stop-mode='))?.split('=')[1] || 'stop';
if (!['stop', 'lease-expiry', 'parent-kill', 'stdin-eof', 'pause', 'capture-freeze', 'capture-recover', 'encode-freeze', 'forward-freeze', 'render-freeze'].includes(mode)) throw new Error('Invalid stop mode');
const freezeMode = ['capture-freeze', 'capture-recover', 'encode-freeze', 'forward-freeze', 'render-freeze'].includes(mode);
const sourceFault = ['capture-freeze', 'capture-recover', 'encode-freeze'].includes(mode);
...
const events = [];
const evidence = { mode, scope, noOsInput: true, testOnlyConsent: true };
let closing = false;
...
try {
  ...
  await promisify(execFile)('xcrun', ['swiftc', '-parse-as-library', ...(sourceFault ? ['-D', 'REMOTE_CONTROL_TEST_FAULTS'] : []), resolve(root, 'scripts/remote-control-screen-source.swift'), '-o', sourcePath], { timeout: 60000 });
  ...
  if (sourceFault) {
    ...
    const sourceArgs = ['--test-freeze-stage', mode === 'encode-freeze' ? 'encode' : 'capture', '--test-freeze-after-ms', '2500'];
  ...
  native = spawn(binary, ['--program', python, '--program-sha256', await digest(python), '--script', script, '--script-sha256', await digest(script), ...engineArgs.flatMap(value => ['--sidecar-arg', value])], { stdio: ['pipe', 'pipe', 'pipe'] });
  ...
  await page.exposeFunction('sendHostSignal', signal => {
    ...
    else if (/ (127\.0\.0\.1|::1) \d+ typ host(?: |$)/.test(signal.candidate.candidate)) send({ type: 'ice', ...signal.candidate });
  });
  await page.evaluate(async ({ binding, key }) => {
    const { RemoteControlPeer } = await import('/src/services/remoteControlPeer.ts');
    const video = document.createElement('video'); video.autoplay = true; video.muted = true; video.tabIndex = 0; video.style.cssText = 'width:640px;height:360px'; document.body.append(video);
  ...
  const pump = async () => {
    for (const event of events.splice(0)) {
      ...
      if (event.event === 'sidecar') {
        ...
        if (kind === 'media-progress') latestProgress = payload;
        if (kind === 'test-fault') {
        ...
        if (kind === 'error') throw new Error(`Engine rejected: ${JSON.stringify(payload)}`);
  ...
  if (scope === 'control') {
    ...
    const queued = await page.evaluate(() => {
      const p = window.supervisorProbe;
      return [p.peer.sendInput({ type: 'button', payload: { x: 0.5, y: 0.5, button: 0, down: true } }),
        p.peer.sendInput({ type: 'button', payload: { x: 0.5, y: 0.5, button: 0, down: false } }),
  ...
  else if (mode !== 'lease-expiry' && !freezeMode) send({ type: 'stop' });
  ...
  if (page) await page.evaluate(() => { const p = window.supervisorProbe; if (p) { clearInterval(p.testHeartbeat); p.peer.end('TEST_CLEANUP'); } }).catch(() => {});
