...
登录页支持用户名或已验证邮箱加密码，也支持已验证邮箱加登录专用验证码。登录发码接口与找回密码一样，对未绑定邮箱返回相同文案；登录验证码不能用于注册、找回密码等其他用途。

找回密码或修改密码成功后，服务端撤销该账号以前的访问令牌和刷新令牌；用户需要重新登录。

新设置的密码（注册、找回或修改）至少 6 位，必须同时包含大写字母、小写字母和数字；允许英文键盘上的半角字母、数字和常用符号，不允许空格、中文、全角字符或表情。旧账号的现有密码仍可用于登录和修改密码时的身份核验。
