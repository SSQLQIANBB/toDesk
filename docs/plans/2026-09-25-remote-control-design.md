...
源码内测版本已进入 `0.3.0-beta.1`，本机最近安装版本为 `0.2.0-beta.2`。新增 [macOS arm64 引擎候选打包](../remote-control-engine-packaging.md)、编译期固定清单、运行时资源校验及[原生启动与协商层](./2026-09-26-remote-control-host-transport.md)；系统 WKWebView 已验证真实画面及开发输入记录链路，VideoToolbox 硬件/软件回退已有实际编码验证。候选包冷启动、正式签名/依赖材料、产品授权接入与真实系统输入仍未完成验收，能力门禁保持关闭。包内链路与开发链路的证据分别记录在实施进度中。
...
当前 TURN 实现与认证核对见 [TURN 接入说明](../remote-control-turn.md)：会话短期凭据接口及浏览器/原生配置已实现，现有服务 UDP/TCP 强制中继通过；现网 REST 共享密钥和跨电脑完整链路仍待核对。
...
- 当前整屏实现把实际内容矩形经认证 IPC 绑定原生显示快照，再发主控 `layout.geometry`。主控分别去除 CSS 与编码帧内黑边，原生使用当前方向的全局逻辑点，不重复乘 Retina 倍率或旋转。首个完整布局和真实解码尺寸未确认前禁止 arm；执行前复查布局，版本不变但字段变化同样结束。P0 对编码尺寸变化也结束重连，后续动态画质需另加安全切换协议。字段、公式与验证范围见[坐标合同](./2026-09-26-remote-control-layout.md)。
