...
原生启动现已增加独占 `RuntimeClaim`：资源校验/创建进程前占用同意，失败或停止按会话与代次清理，成功验证实际 DTLS 与连接凭据后只转移一次给运行时。开发媒体测试已复用该层；正式产品适配器仍待接通，详见[启动与协商边界](./2026-09-26-remote-control-host-transport.md)。
