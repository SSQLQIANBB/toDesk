...
.chat-voice__wave { display: flex; align-items: center; gap: 2px; height: 16px; flex: none; }
.chat-voice__wave i { display: block; width: 2px; height: 12px; background: currentColor; border-radius: 2px; }
.chat-voice__wave i:nth-child(2) { height: 16px; }
.chat-voice__wave i:nth-child(3) { height: 8px; }
.chat-voice:hover { opacity: .85; }
.chat-voice__duration { flex: none; font-size: 13px; font-variant-numeric: tabular-nums; }
.chat-voice--playing .chat-voice__wave i { animation: voice-wave 1s ease-in-out infinite; }
.chat-voice--playing .chat-voice__wave i:nth-child(2) { animation-delay: .12s; }
.chat-voice--playing .chat-voice__wave i:nth-child(3) { animation-delay: .24s; }
@keyframes voice-wave { 0%, 100% { transform: scaleY(.3); } 50% { transform: scaleY(1); } }
@media (prefers-reduced-motion: reduce) { .chat-voice--playing .chat-voice__wave i { animation: none; } }
</style>
